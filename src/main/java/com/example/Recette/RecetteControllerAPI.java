...
public class RecetteControllerAPI {
    ...
    public ResponseEntity<String> deleteRecette(@RequestBody RecetteDelete RecetteDelete)
    {
        ...
        if(RecetteDelete==null || RecetteDelete.getTitle()=="" || RecetteDelete.getTitle()==null){
            return new ResponseEntity<String>("Your value is empty",HttpStatus.INTERNAL_SERVER_ERROR);
        }

        int val = recetteDao.countTitle(RecetteDelete.getTitle());
        if(val==0){
            return new ResponseEntity<String>("You don't have recette",HttpStatus.INTERNAL_SERVER_ERROR);
        }

        Recette recette = recetteDao.findByTitle(RecetteDelete.getTitle());
        recetteDao.delete(recette);
        return new ResponseEntity<String>("The recette is deleted",HttpStatus.OK);
